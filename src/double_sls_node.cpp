...
bool open_loop_ctrl_enabled = true;
...
const double r_1 = 5.0;
const double fr_1 = 1.0;
const double c_1 = -3;
const double ph_1 = 0;
const double r_2 = 5.0;
...
void gazeboCb(const gazebo_msgs::LinkStates::ConstPtr& msg, ros::Publisher* attitude_setpoint_pub_0, ros::Publisher* attitude_setpoint_pub_1){
    ...
    if(dea_enabled){
        ...
        else applyDEAController(state18, dea_xi4, dea_k, dea_param, dea_ref, ros::Time::now().toSec() - dea_start_time);
        ...
        applyDEAController(state18, dea_xi4, dea_k, dea_param, dea_ref, 0);
    }
...
void force_rate_convert(double controller_output[3], mavros_msgs::AttitudeTarget &attitude, int uav){
    ...
    bool rpy_debug_enabled = false;
    if(rpy_debug_enabled) ROS_INFO_STREAM("Target RPY:" << roll << " " << pitch << " " << yaw);
